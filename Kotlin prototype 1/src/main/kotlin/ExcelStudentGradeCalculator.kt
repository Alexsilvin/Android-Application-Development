...
data class ExcelStudent(val name: String, val score: Double, val grade: Char)

private data class ImportResult(
    val students: List<ExcelStudent>,
...
private fun readStudentsFromExcel(file: File): ImportResult {
    FileInputStream(file).use { inputStream ->
        WorkbookFactory.create(inputStream).use { workbook ->
            ...
            val students = mutableListOf<ExcelStudent>()
            ...
            for (rowIndex in (sheet.firstRowNum + 1)..sheet.lastRowNum) {
                ...
                students += ExcelStudent(name, score, getLetterGrade(score))
            }
...
private fun printStudentTable(students: List<ExcelStudent>) {
...
private fun exportStudentsToExcel(students: List<ExcelStudent>, outputFile: File) {
